%   This script runs the Psychtoolbox with an antisaccade task recorded via Eyelink 1000
%   with sumultaneous EEG recordings

%   Copyright (C) April 2020, last modified June 2020
%   A. Koenig, J.Waldthaler,A. Sperlich, David Pedrosa
...
% Information parameter for evaluation: are these trials monocular or
% binocular
isbinocular = 1;
...
%Setup-Parameter - in cm - EEG Lab, same monitor as PsychoBox3
monitorwidth = 53.0;
monitorheight = 29.9;
monitordistance = 70; % 58cm to the window, 12 cm to the monitor, Lab-protocol says its 56 cm to the window, can't confirm
...
if exist(savePath,'dir')~=7 %%% Existenz des Verzeichnisse pr¸fen, sonst erstellen
      mkdir('.',savePath);
end
...
%Initialize DataPixx Monitor in the lab
% if usingLab == 1
%     Datapixx('Open');
...
% [window, windowRect] = Screen('OpenWindow', screenNumber, background, [], 16, 2, [], []);

% Folgendes funktioniert im EEG Labor und neuerdgins in der PsychoBox3
% (neue hardware)
...
for trial = 1:numTrials
      ...
      % Benachrichtigung an EyeLink ob Fixationspunk rechts oder links
      ...
      % Der Fixationspunkt soll ein grˆﬂerer Punkt mit einem kleinen Punkt im
      ...
      for frame = 1: grTimeFrames
            ...
            if keyCode(escapeKey)
                  ...
                  % Stop DataPixx Monitor in the lab
                  %             if usingLab==1
                  %                 Datapixx('Close');
                  %             end
      ...
      for frame = 1 : stimTimeFrames
            ...
            %         not used anymore
            %         % Leertaste beendet den Trial, f¸r den Fall, dass der Punkt nicht
            %         % gesehen wurde, weil er im blinden Fleck war
            %         if keyCode(space)
            %             localization = 0; %Wenn Stimulus nicht gesehen wurde, kennzeichne dies durch 0
            %             break;
            %         end
            %
            % Falls der Test nicht als "nicht gesehen" abgebrochen wurde, setze den Stimulus als gesehen im letzten Frame
            %         if frame == presTimeFrames+saccadeTimeFrames+respTimeFrames
            %             localization = 1; %Wenn Stimulus gesehen wurde, kennzeichne dies durch 1
            %         end
      ...
      data(trial).isbinocular = isbinocular;                                  % Speichert ob das der binokulae Versuchsteil war oder nicht
